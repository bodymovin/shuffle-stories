...
export function buildHash(totalOptions: number, direction: directions, prefix: string, hash: string): string {
  let currentHashIndex = 0
  if (hash) {
    const hashParts = hash.split('-')
    if (hashParts.length) {
      const lastPart = parseInt(hashParts[hashParts.length - 1])
      if (!isNaN(lastPart)) {
        if (direction < 0) {
          if (lastPart > 0) {
